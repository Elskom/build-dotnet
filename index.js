...
class Action {
    ...
    run() {
        ...
        this._executeInProcess(`dotnet build -c Release -p:Platform=AnyCPU -bl:./msbuild.binlog ${this.restore === true ? "--no-restore" : ""}${this.solutionFile === "" ? this.solutionFile : ` ${this.solutionFile}`}`, `build failed`)
        if (this.test === "true") {
            this._executeInProcess(`dotnet test -c Release -p:Platform=AnyCPU --no-build${this.solutionFile === "" ? this.solutionFile : ` ${this.solutionFile}`}`, `testing failed`)
        }
        ...
        if (this.pack === "true") {
            ...
            this._executeInProcess(`dotnet pack --no-build -c Release -p:Platform=AnyCPU${this.solutionFile === "" ? this.solutionFile : ` ${this.solutionFile}`}`, `packing failed`)
        }
